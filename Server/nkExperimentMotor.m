function o = nkExperimentMotor_1(maxcue,session_num,trial_num,break_time)
%Usage
% o=nkExperimentMotor_1(num_cues,num_sessions,num_trials,break_time)
...
%% Startup dialog
if(nargin<1)
  maxcue=input('Uyaricilarin sayisi [1-6]:');
...
clf
set(fig,'Position',[309 0 676 677]);
set(fig,'Color',[1 1 1]);
...
%fixation point
annotation(fig,'ellipse',[0.500 0.4860 0.030 0.030],...
  'FaceColor',[0 0 0]);

%indicator boxes
hbox={[0.1164 0.6177 0.2342 0.2819],
      [0.6823 0.6177 0.2342 0.2819],
      [0.4000 0.6177 0.2342 0.2819],      
      [0.1164 0.1394 0.2342 0.2819],
      [0.6823 0.1394 0.2342 0.2819],
      [0.4000 0.1394 0.2342 0.2819],
      [0.4660 0.4515 0.1000 0.1000]};      

%fixation box  
annotation(fig,'rectangle',hbox{7},'LineWidth',5,...
    ...
    function update_state(cuestate)
        ...
        if(cuestate==0)
            hState(1)=annotation(fig,...
              'rectangle',hbox{7},'LineWidth',5,...
              'FaceColor','flat','Color',[1 0 0]);            
        elseif(cuestate>0 && cuestate<=length(imgs))
            hState(1)=annotation(fig,...
              'rectangle',hbox{cuestate},'LineWidth',5,...
              'FaceColor','flat','Color',[1 0 0]);            
        elseif(cuestate==90 || cuestate==99)
            hState(1)=annotation(fig,'textbox',[0.1154 0.4365 0.7751 0.2437],...
